##########
# python eval2.py --dataset Set5 --cuda
##########
...
parser.add_argument('--sparsity', type=float, default=0.5, metavar='M',
                        help='Sparsity for the config_list parameter which is passed into the pruner (default: 0.5)')
...
use_cuda = not opt.cuda and torch.cuda.is_available()
device = torch.device("cuda" if use_cuda else "cpu")
model = torch.load("model/" + "model_epoch_L1_No_Finetune.pth").to(device)
